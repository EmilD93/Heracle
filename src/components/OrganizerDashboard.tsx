        ...
        <div className="lg:col-span-2 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-[2rem] shadow-sm border border-slate-100/80 dark:border-slate-700/80 overflow-hidden">
          <div className="p-6 border-b border-slate-100/80 dark:border-slate-700/80 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button className="px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl text-sm font-bold hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors focus:outline-none">All</button>
              <button className="px-4 py-2 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-xl text-sm font-bold transition-colors focus:outline-none">Published</button>
              <button className="px-4 py-2 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-xl text-sm font-bold transition-colors focus:outline-none">Drafts</button>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 dark:bg-slate-900/50">
                  <th className="py-4 px-6 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Event</th>
                  <th className="py-4 px-6 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Date</th>
                  <th className="py-4 px-6 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Status</th>
                  <th className="py-4 px-6 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Capacity</th>
                  <th className="py-4 px-6 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-right">Actions</th>
                </tr>
              </thead>
                    ...
                    >
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="font-bold text-slate-800 dark:text-slate-100 text-[15px] mb-0.5">{event.title}</div>
                            <div className="text-sm font-medium text-slate-500 dark:text-slate-400">{event.category}</div>
                          </div>
                        </div>
                      </td>
                      <td className="py-4 px-6">
                        <div className="text-[15px] font-semibold text-slate-700 dark:text-slate-300">{event.date.split('•')[0]}</div>
                        <div className="text-sm font-medium text-slate-500 dark:text-slate-400">{event.date.split('•')[1]}</div>
                      </td>
                      <td className="py-4 px-6">
                        <span
                          className={cn(
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          <div className="flex-1 h-2 w-24 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                            <div className={cn('h-full rounded-full', isFull ? 'bg-amber-500' : 'bg-emerald-500')} style={{ width: `${percentage}%` }} />
                          </div>
                          <span className="text-sm font-bold text-slate-600 dark:text-slate-300 w-12 text-right">{event.registered}/{event.capacity}</span>
                        </div>
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-2 transition-opacity">
                          {event.status === 'Draft' && (
                            <button
                              ...
                              className="p-2 text-slate-400 dark:text-slate-500 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 rounded-lg transition-colors focus:outline-none"
                          ...
                          <button
                            ...
                            className="p-2 text-slate-400 dark:text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-lg transition-colors focus:outline-none"
                          ...
                          {event.status !== 'Cancelled' && (
                            <button
                              ...
                              className="p-2 text-slate-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors focus:outline-none"
        ...
        <div className="flex flex-col gap-8">
              ...
              {[
                ...
                <div key={i} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[15px] font-bold text-slate-800 dark:text-slate-100 truncate">{reg.name}</div>
                    <div className="text-sm font-medium text-slate-500 dark:text-slate-400 truncate">{reg.event}</div>
                  </div>
                  <div className="text-xs font-bold text-slate-400 dark:text-slate-500 shrink-0">{reg.time}</div>
                </div>
              ))}
              <button className="mt-auto pt-4 w-full text-sm font-bold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition-colors focus:outline-none">
                View All Activity
              </button>
            </div>
          </div>
          ...
          <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-[2rem] shadow-sm border border-slate-100/80 dark:border-slate-700/80 overflow-hidden flex flex-col">
            ...
            <div className="p-6 flex-1 flex flex-col gap-5">
              ...
              <button className="mt-auto pt-2 w-full text-sm font-bold text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 transition-colors focus:outline-none">
                Manage Waitlists
              </button>
            </div>
          </div>
